...
class ExpTreeBase:
    def __init__(
        ...
        self.double_constant_count = 0 # avoid double constant in a binary operation
    ...
    def random_fill(self) -> List[str]:
        ...
        while not self.is_terminal():
            ...
            op_list.append(op)
            self.add_op(op)
        complex_val = sum(self.complexity[op] for op in self.op_list)
        return self, op_list, complex_val
...
class ExpTree(ExpTreeBase):
    ...
    def add_op(self, op: str) -> Tuple[str, int, int, int]:
        ...
        arity = self.arity_dict.get(op, 0)
        # reset if a non-constant operation is added
        if arity > 0:
            self.double_constant_count = 0
    ...
    def update_available_ops(self):
        ...
        # at max depth, only allow leaf nodes
        ...
        for stack_entry in self.stack:
            ...
            if op == 'sin' or op == 'cos':
                ...
                break
            if self.double_constant_count >= self.arity_dict.get(op, 0) - 1:
                # unitary operation can not be followed by a constant
                # binary operation can not be followed by two constants
                available_ops = [op for op in available_ops if op != 'C' and op != 'R']
                break
        if self.stack:
