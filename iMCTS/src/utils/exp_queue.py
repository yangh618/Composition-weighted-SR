...
class Queue_Base(ABC):
    ...
    def __init__(self, max_size: int):
        ...
        # two rewards: train_reward and valid_reward, we sort by valid_reward
        self.list: SortedList[Tuple[Any, float, float]] = SortedList(key=lambda x: -x[2])  # (state, train_reward, valid_reward)
...
class Exp_Queue(Queue_Base):
    ...
    def append(self, state: Any, train_reward: float, valid_reward: float = 0.0, threshold: float = 1e-5) -> bool:
        ...
        if math.isinf(valid_reward) or math.isnan(valid_reward):
            return False

        lst = self.list
        if (len(lst) >= self.max_size) and (valid_reward <= self.min_reward):
        ...
        pos = reward_values.bisect_left(valid_reward)
        if pos > 0 and abs(reward_values[pos - 1] - valid_reward) < threshold:
            return False
        if pos < len(reward_values) and abs(reward_values[pos] - valid_reward) < threshold:
            return False

        lst = self.list
        if len(lst) < self.max_size:
            lst.add((state, train_reward, valid_reward))
            reward_values.add(valid_reward)
        else:
            # Fast check: if not better than current minimum, discard.
            if valid_reward <= self.min_reward:
                return False
            # Remove worst (last element due to descending order via key)
            removed_state, _, removed_reward = lst.pop(-1)
        ...
        self.min_reward = lst[-1][2] if lst else float('-inf')
        return True
